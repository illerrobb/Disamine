...
const CandidateDetailPanel = ({ candidate, position, evaluation, selected, onBack, onChoose }: {
  ...
  return <div data-testid="candidate-detail-panel" className="space-y-5">
    ...
    <div><div className="text-xs font-bold uppercase tracking-wider text-violet-600">Candidato selezionato</div><div className="mt-2 text-[10px] font-bold uppercase text-slate-500">{candidateProfile(candidate)}</div><h3 className="mt-0.5 text-lg font-bold text-slate-900">{candidate.nominativo}</h3><div className="font-mono text-[10px] text-slate-400">{candidate.id}</div></div>
...
export const PositionDetailPanel = ({ snapshot, candidates, positions, evaluations, activeChoices, scenario, previewChoice, previewAnalysis, committedAnalysis, onPreview, onInspect, onChoose }: {
  ...
  return <div data-testid="position-detail-panel" className="space-y-5">
    <div><div className="text-xs font-bold uppercase tracking-wider text-blue-600">Posizione</div><h3 className="mt-1 text-xl font-bold text-slate-900">{position.code}</h3><p className="text-sm font-semibold text-slate-700">{position.title || "Titolo non indicato"}</p><p className="mt-1 flex flex-wrap gap-x-1 text-[10px] font-bold uppercase text-blue-700">{[position.rankReq, position.role, position.catSpecQualReq].filter(Boolean).map(value => <span key={value}>{value}</span>)}</p></div>
