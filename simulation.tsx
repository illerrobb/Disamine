...
const CandidateChoice = ({ candidate, position, evaluation, selected, occupied, onPreview, onInspect, onChoose }: {
  ...
  return <div className={`group relative min-w-[220px] max-w-[300px] rounded-xl border p-3 text-left transition-all ${selected ? "border-blue-600 bg-blue-600 text-white shadow-sm" : occupied ? "border-amber-200 bg-amber-50" : "border-slate-200 bg-white hover:border-blue-400 hover:shadow-sm"}`}>
    <button type="button" aria-label={`Anteprima ${candidate.nominativo} per ${position.code}`} onPointerEnter={() => onPreview(true)} onPointerLeave={() => onPreview(false)} onFocus={() => onPreview(true)} onBlur={() => onPreview(false)} onClick={onInspect} className="block w-full rounded-lg text-left outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2">
      <span className="flex items-start justify-between gap-2"><span className="min-w-0"><span className={`block text-[9px] font-bold uppercase tracking-wide ${selected ? "text-blue-100" : "text-slate-400"}`}>Persona segnalata</span><span className="block truncate text-xs font-bold">{candidate.nominativo}</span><span className={`block text-[9px] ${selected ? "text-blue-100" : "text-slate-500"}`}>{candidate.id} · {[candidate.rank, candidate.role, candidate.category, candidate.specialty].filter(Boolean).join(" · ") || "Profilo n.d."}</span><span className={`mt-1 block text-[9px] font-bold uppercase tracking-wide ${selected ? "text-blue-100" : "text-slate-400"}`}>Per la posizione</span><span className="block truncate text-[11px]">{position.code} · {position.title}</span><span className={`mt-1 block text-[9px] ${selected ? "text-blue-100" : "text-slate-500"}`}>Personale previsto: {position.plannedPersonnel || "n.d."} · Ruolo CAT/SPEC/QUAL: {position.catSpecQualReq || "n.d."}</span></span>{selected && <Check className="h-4 w-4 shrink-0" />}</span>
      ...
      <span className={`mt-1 block text-[9px] ${selected ? "text-blue-100" : "text-slate-400"}`}><b>Disamina requisiti:</b> {evaluationLabel}</span>
    </button>
    <button type="button" aria-label={`${selected ? "Rimuovi" : "Assegna"} ${candidate.nominativo} ${selected ? "dallo" : "allo"} scenario per ${position.code}`} onClick={onChoose} className={`mt-3 w-full rounded-lg px-2 py-1.5 text-[10px] font-bold outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2 ${selected ? "bg-white text-blue-700 hover:bg-blue-50" : "bg-blue-600 text-white hover:bg-blue-700"}`}>{selected ? "Rimuovi dallo scenario" : "Assegna allo scenario"}</button>
...
const CandidateDetailPanel = ({ candidate, position, evaluation, selected, onBack, onChoose }: {
  ...
  return <div data-testid="candidate-detail-panel" className="space-y-5">
    ...
    <div className="rounded-xl border border-blue-100 bg-blue-50 p-3"><div className="text-[10px] font-bold uppercase text-blue-600">Posizione selezionata</div><div className="font-mono text-sm font-bold text-blue-800">{position.code}</div><div className="text-xs text-slate-600">{position.title} · {position.entity}</div><div className="mt-1 text-[10px] text-blue-800"><b>Personale previsto:</b> {position.plannedPersonnel || "n.d."}</div><div className="text-[10px] text-blue-800"><b>Ruolo CAT/SPEC/QUAL:</b> {position.catSpecQualReq || "n.d."}</div></div>
    <div><div className="text-xs font-bold uppercase tracking-wider text-violet-600">Candidato selezionato</div><h3 className="mt-1 text-lg font-bold text-slate-900">{candidate.nominativo}</h3><div className="font-mono text-[10px] text-slate-400">{candidate.id}</div></div>
    <dl className="space-y-2 text-xs"><div><dt className="font-bold uppercase text-slate-400">Anagrafica</dt><dd className="text-slate-700">{[candidate.rank, candidate.role, candidate.category, candidate.specialty].filter(Boolean).join(" · ") || "Non indicata"}</dd></div><div><dt className="font-bold uppercase text-slate-400">Ente</dt><dd className="text-slate-700">{candidate.serviceEntity || "Non indicato"}</dd></div><div><dt className="font-bold uppercase text-slate-400">Compatibilità</dt><dd className="font-bold text-slate-700">{getFit(position, evaluation)}%</dd></div><div><dt className="font-bold uppercase text-slate-400">Stato valutazione</dt><dd className="text-slate-700">{statusLabel} · {requirementSummary(position, evaluation)}</dd></div></dl>
...
export const PositionDetailPanel = ({ snapshot, candidates, positions, evaluations, activeChoices, scenario, previewChoice, previewAnalysis, committedAnalysis, onPreview, onInspect, onChoose }: {
  ...
  return <div data-testid="position-detail-panel" className="space-y-5">
    ...
    <dl className="space-y-2 text-xs"><div><dt className="font-bold uppercase text-slate-400">Ente</dt><dd className="text-slate-700">{position.entity || "Non indicato"}</dd></div><div><dt className="font-bold uppercase text-slate-400">Ruolo posizione</dt><dd className="text-slate-700">{position.role || "Non indicato"}</dd></div><div><dt className="font-bold uppercase text-slate-400">Personale previsto</dt><dd className="text-slate-700">{position.plannedPersonnel || "Non indicato"}</dd></div><div><dt className="font-bold uppercase text-slate-400">Ruolo CAT/SPEC/QUAL</dt><dd className="text-slate-700">{position.catSpecQualReq || "Non indicato"}</dd></div><div><dt className="font-bold uppercase text-slate-400">Stato amministrativo</dt><dd className="font-semibold text-slate-700">{administrativeStatusLabel(snapshot)}</dd></div><div><dt className="font-bold uppercase text-slate-400">Requisiti principali</dt><dd className="mt-1 text-slate-600">{requirements.length ? requirements.map(requirement => requirement.text).join(" · ") : [position.rankReq, position.catSpecQualReq, position.englishReq, position.nosReq].filter(Boolean).join(" · ") || "Nessun requisito indicato"}</dd></div></dl>
...
export const SimulationDashboard = ({ candidates, positions, evaluations, researchId }: {
  ...
  const scenarioPositions = useMemo(() => positionsInConfigScope(positions, activeScenario?.config), [positions, activeScenario?.config]);
    ...
    <div className="flex h-full min-h-0 flex-col bg-slate-100">
      ...
      <div className={`min-h-0 flex-1 p-4 ${compareOpen ? "overflow-auto" : "flex gap-4"}`}>
        ...
        <aside className="flex w-72 shrink-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 p-4"><div className="flex items-center justify-between"><div><h2 className="text-sm font-bold text-slate-900">Scelte dello scenario</h2><p className="mt-0.5 text-xs text-slate-400">Clicca il contenuto per il dettaglio</p></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">{activeChoices.length}</span></div></div>
          <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3" onClick={event => { if (event.target === event.currentTarget) setSelectedChoiceId(null); }}>
            {activeChoices.map((choice, index) => {
              ...
              return <div key={choice.id} className="group w-full rounded-xl border border-slate-200 p-2 transition-all hover:border-slate-300 hover:shadow-sm"><button type="button" aria-label={`Approfondisci ${candidate?.nominativo ?? choice.candidateId} per ${choice.positionId}`} onClick={() => inspectCandidate(choice.candidateId, choice.positionId)} className="flex w-full items-start gap-2 rounded-lg p-1 text-left outline-none focus-visible:ring-2 focus-visible:ring-blue-400"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-[10px] font-bold text-slate-500">{index + 1}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-slate-800">{candidate?.nominativo ?? choice.candidateId}</span><span className="mt-1 flex items-center gap-1.5 text-xs text-slate-500"><ArrowRight className="h-3 w-3 text-blue-500" /><span className="font-mono font-bold text-blue-700">{position?.code}</span><span className="truncate">{position?.entity}</span></span></span></button><button type="button" aria-label={`Rimuovi ${candidate?.nominativo ?? choice.candidateId} dallo scenario per ${choice.positionId}`} onClick={() => updateActiveChoices(activeChoices.filter(item => item.id !== choice.id))} className="mt-2 w-full rounded-lg border border-rose-100 px-2 py-1 text-[10px] font-bold text-rose-700 outline-none hover:bg-rose-50 focus-visible:ring-2 focus-visible:ring-rose-400">Rimuovi dallo scenario</button></div>;
            })}
