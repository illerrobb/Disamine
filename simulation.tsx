...
const CandidateDetailPanel = ({ candidate, position, evaluation, selected, onBack, onChoose }: {
  ...
  return <div data-testid="candidate-detail-panel" className="space-y-5">
    ...
    <div className="rounded-xl border border-blue-100 bg-blue-50 p-3"><div className="text-[10px] font-bold uppercase text-blue-600">Posizione selezionata</div><div className="font-mono text-sm font-bold text-blue-800">{position.code}</div><div className="truncate text-xs text-slate-600">{position.title} · {position.entity}</div></div>
...
export const SimulationDashboard = ({ candidates, positions, evaluations, researchId }: {
  ...
  const positionRoles = useMemo(() => Array.from(new Set(positions.map(position => position.role).filter(Boolean))).sort((a, b) => a.localeCompare(b, "it")), [positions]);
